...
Minimal local Mac bridge worker for Sage.
...
- `SAGE_DEVICE_ID` (optional, default `mac-${pid}`)
- `SAGE_ATTESTATION` (optional, default `verified:dev-local`)
...
- `SAGE_BRIDGE_STATE_FILE` (optional, default `~/.sage/mac-bridge-state.json`)
...
- This is a scaffold bridge for local development, not yet a packaged production app.
