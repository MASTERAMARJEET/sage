...
async function main(): Promise<void> {
  ...
  for (;;) {
    ...
    if (config.runOnce) {
      console.log("[sage-mac-bridge] run-once cycle complete");
      return;
    }
...
function loadConfig(): BridgeConfig {
  ...
  const deviceId = process.env.SAGE_DEVICE_ID ?? `mac-${process.pid}`;
  ...
  const attestation = process.env.SAGE_ATTESTATION ?? "verified:dev-local";
  const allowedRoots = (process.env.SAGE_ALLOWED_ROOTS ?? process.cwd())
    .split(",")
    .map((entry) => path.resolve(entry.trim()))
    .filter((entry) => entry.length > 0);
  const stateFile = path.resolve(
    process.env.SAGE_BRIDGE_STATE_FILE ?? path.join(homedir(), ".sage", "mac-bridge-state.json")
  );
  return {
...
main().catch((error) => {
  console.error("[sage-mac-bridge] fatal", error);
  process.exitCode = 1;
});
